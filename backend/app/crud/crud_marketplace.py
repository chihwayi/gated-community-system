...
class CRUDMarketplace(CRUDBase[MarketplaceItem, MarketplaceItemCreate, MarketplaceItemUpdate]):
    def create_with_seller(self, db: Session, *, obj_in: MarketplaceItemCreate, seller_id: int) -> MarketplaceItem:
    ...
    def get_multi_by_category(self, db: Session, *, category: str, skip: int = 0, limit: int = 100) -> List[MarketplaceItem]:
    ...
    def get_available(self, db: Session, skip: int = 0, limit: int = 100) -> List[MarketplaceItem]:
        return db.query(MarketplaceItem).filter(
            MarketplaceItem.status == MarketplaceItemStatus.AVAILABLE
        ).order_by(MarketplaceItem.created_at.desc()).offset(skip).limit(limit).all()

    def get_by_seller(self, db: Session, seller_id: int) -> List[MarketplaceItem]:
        return db.query(MarketplaceItem).filter(MarketplaceItem.seller_id == seller_id).all()
