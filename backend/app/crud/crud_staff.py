...
class CRUDStaff(CRUDBase[Staff, StaffCreate, StaffUpdate]):
    def create_staff(self, db: Session, *, staff: StaffCreate) -> Staff:
    ...
    def get_active_staff(self, db: Session) -> List[Staff]:
        return db.query(Staff).filter(Staff.status == StaffStatus.ACTIVE).all()
...
class CRUDStaffAttendance(CRUDBase[StaffAttendance, StaffAttendanceCreate, StaffAttendanceUpdate]):
    ...
    def get_all_attendance(self, db: Session, skip: int = 0, limit: int = 100, start_date: datetime = None, end_date: datetime = None) -> List[StaffAttendance]:
