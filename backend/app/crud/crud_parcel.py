...
def create_parcel(db: Session, parcel: schemas.ParcelCreate) -> Parcel:
...
def get_parcels(db: Session, skip: int = 0, limit: int = 100, recipient_id: Optional[int] = None, status: Optional[ParcelStatus] = None) -> List[Parcel]:
...
def update_parcel_status(db: Session, parcel_id: int, status: ParcelStatus) -> Optional[Parcel]:
